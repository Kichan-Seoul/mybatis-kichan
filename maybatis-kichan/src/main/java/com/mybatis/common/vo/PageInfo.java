...
public class PageInfo {
	private int totalRecord; 	//현재 총 게시글의 개수 (62)
	private int nowPage;		//현재 페이지(즉, 사용자가 요청한 페이지)
	private int numPerPage; 	//현재 페이지에 보여줄 게시글의 개수
	private int pagePerBlock;	//페이지 하단에 보여질 페이징바의 페이지 개수
	
	// 위의 4개를 가지고 값을 구함
	private int totalPage; 		//가장 마지막 페이지(총 페이지 수)
